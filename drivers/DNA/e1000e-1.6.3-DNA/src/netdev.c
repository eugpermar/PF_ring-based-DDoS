...
#ifdef ENABLE_DNA
#undef DRV_EXTRAVERSION
#define DRV_EXTRAVERSION  "-DNA"
#endif
...
#define rx_csum	(rx_desc->wb.lower.hi_dword.csum_ip.csum)
		/* Receive Checksum Offload */
#ifdef NETIF_F_RXHASH
		if (netdev->features & NETIF_F_RXHASH) {
			e1000_rx_checksum(adapter, staterr, 0, skb);
			skb->rxhash =
			    le32_to_cpu(rx_desc->wb.lower.hi_dword.rss);
		} else {
#endif
			e1000_rx_checksum(adapter, staterr,
					  le16_to_cpu(rx_csum), skb);
#ifdef NETIF_F_RXHASH
		}
#endif
...
#endif
{
	...
	if(hook && (hook->magic == PF_RING) && adapter->dna.dna_enabled) {
	  ...
	  if(0) printk(KERN_INFO
		       "DNA: e1000_clean_rx_irq(%s)[id=%d]\n",
